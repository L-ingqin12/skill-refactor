...
#### Layer 3: Skill 语法检查 (Must Pass — 类似代码 lint)

重构后必须对每个 skill 文件做语法检查——这是 skill 的 "compiler check"：

```
检查项:
□ YAML frontmatter 有效: python3 -c "import yaml; yaml.safe_load(open('file'))"
  → 失败: 语法错误 → 立即修复
□ 必填字段: name 或 description 至少一个存在
□ 无断链: 所有引用的 scripts/references/其他 skill 路径实际存在
□ Markdown 结构: 无孤立标题 (### 没有更高级别标题)
□ description ≤50 words (wc -w 验证)
...
**通过标准**: 0 语法错误 + 0 断链 + 0 孤立标题。

#### Layer 4: 精简完备性 (Nice to Pass)
- 每个 skill body ≤ 500 行
- 无重复段落（跨 skill 比较）
...
详见 `references/refactoring_patterns.md`，包含：
- 7 种基础重构模式（Pipeline Merge / Parameterized Variant / Subroutine Extraction / Layered Split / Namespace Cleanup / Dead Skill Removal / Trigger Precision Tuning）
- **代码重构 → Skill 重构对照表**（Extract Method / Inline Method / Rename / Move Method / Extract Class / Replace Conditional with Polymorphism）
- **重构时机判断**（Rule of Three + 安全条件）
